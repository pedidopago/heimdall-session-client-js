...
export function midVerify(req: Request, res: Response, next: NextFunction) {
  ...
  if (hkey !== process.env.HEIMDALL_SESSION_HEADER_KEY) {
    res
      .status(500)
      .json({ error: "cannot trust Heimdall headers (hkey mismatch)" });
    return;
  }
...
export function midValidCustomer(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export function newMidWithAgentPermission(
  role: string,
  perm: string
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    zlib.inflate(rawd, (err, buffer) => {
      ...
      if (!hasRolePerm(role, perm, rperms)) {
        res
          .status(403)
          .json({ error: "not authorized (missing role or permission)" });
        return;
      }
      next();
    });
...
export function getAgentName(req: Request): string {
  const val = req.header("x-heimdall-msagent-name");
  if (!val) {
    return "";
  }
  return val;
